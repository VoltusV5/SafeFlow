#!/usr/bin/env python3
"""Смена аватарки бота (online/offline) для systemd ExecStartPre / ExecStopPost.

Источники по умолчанию (папка assets/):
  online  — «bot_avatar_online.jpg»
  offline — «bot_avatar_offline.jpg»

Переопределение: BOT_AVATAR_ONLINE / BOT_AVATAR_OFFLINE в .env (любой путь к PNG или JPEG).
...
def _fallback_assets_jpeg(online: bool) -> Path:
...
def _ensure_jpeg_placeholder(path: Path, *, online: bool) -> None:
    ...
    try:
        ...
        logger.error("Pillow не установлен — положите свои изображения в %s", path.parent)
        raise
    w, h = 640, 640
    if online:
        bg = (39, 174, 96)
        ...
        im = Image.new("RGB", (w, h), bg)
        dr = ImageDraw.Draw(im)
        dr.rounded_rectangle((140, 140, 500, 500), radius=40, fill=(127, 140, 141))
...
def _resolve_source_path(online: bool) -> Path:
    ...
    s = get_settings()
    raw = (s.bot_avatar_online_path if online else s.bot_avatar_offline_path).strip()
    if not raw:
        raw = str(
            _PROJECT_ROOT / "assets" / ("bot_avatar_online.jpg" if online else "bot_avatar_offline.jpg")
        )
...
async def _set_avatar(online: bool) -> None:
    ...
    try:
        ...
        try:
            ...
            logger.info("аватарка установлена (%s): %s", "online" if online else "offline", path)
        except Exception as e:
            logger.warning("не удалось установить аватарку (возможно, лимит): %s", e)
    finally:
...
def main() -> None:
    if len(sys.argv) != 2 or sys.argv[1] not in ("online", "offline"):
        print("Использование: set_bot_avatar.py online|offline", file=sys.stderr)
        sys.exit(2)
