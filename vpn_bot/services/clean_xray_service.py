...
_CLEAN_XRAY_CONFIG_PATH = Path("/root/vpn-telegram-bot/deploy/clean-xray/config.json")
...
def _resolve_host() -> str:
    ...
    if not ep:
        raise ContainerIssueError("Не задан CLEAN_XRAY_HOST / PUBLIC_HOST / WG_ENDPOINT")
    if "]:" in ep:
...
def _ensure_container_running() -> str:
    ...
    status = _run(["docker", "inspect", "-f", "{{.State.Status}}", ctr], timeout=30)
    if status != "running":
        raise ContainerIssueError(f"clean-xray контейнер не запущен: {ctr} ({status})")
    return ctr
...
def _add_user_to_config(inbound_tag: str, client_obj: dict) -> None:
    ...
    if not inbound:
        raise ContainerIssueError(f"Inbound {inbound_tag} не найден в config.json")
    ...
    # Validate using a temporary container or the running one if it has xray binary
    # We can just use the running one: docker exec vpn-clean-xray xray -test -config /etc/xray/config.json
    val = subprocess.run(["docker", "exec", ctr, "xray", "-test", "-config", "/etc/xray/config.json"], capture_output=True)
    if val.returncode != 0:
        raise ContainerIssueError(f"Ошибка валидации конфига: {val.stderr.decode()[:200]}")
...
def _get_reality_params(s: Settings) -> tuple[str, str, str]:
    ...
    if not pbk or not sid:
        raise ContainerIssueError(
            "Для REALITY задайте CLEAN_XRAY_REALITY_PUBLIC_KEY и CLEAN_XRAY_REALITY_SHORT_ID"
        )
    return pbk, sid, sni


def _build_reality_query(pbk: str, sid: str, sni: str) -> str:
    return f"security=reality&pbk={quote(pbk)}&sni={quote(sni)}&sid={quote(sid)}&fp=chrome"


def _remove_reality_from_config(inbound_tag: str) -> None:
    """Helper to ensure the inbound has NO REALITY (for VMess if it breaks clients)."""
    ctr = _ensure_container_running()
...
def _ensure_reality_in_config(inbound_tag: str, pbk: str, sid: str, sni: str) -> None:
    """Helper to ensure the inbound has REALITY enabled in config.json (for Trojan/VMess)."""
    ctr = _ensure_container_running()
    ...
    for ib in doc.get("inbounds", []):
        if ib.get("tag") == inbound_tag:
            ss = ib.get("streamSettings", {})
            if ss.get("security") != "reality":
                vless_ib = next((x for x in doc["inbounds"] if x.get("tag") == "in-vless"), None)
                priv_key = vless_ib["streamSettings"]["realitySettings"]["privateKey"]
...
def issue_clean_xray_vmess_url() -> tuple[str, str]:
    ...
    raw = json.dumps(vmess_obj, ensure_ascii=False, separators=(",", ":")).encode("utf-8")
...
def issue_clean_xray_ss_url() -> tuple[str, str]:
    ...
    userinfo = base64.urlsafe_b64encode(f"{method}:{pwd}".encode("utf-8")).decode("ascii").rstrip("=")
    url = f"ss://{userinfo}@{host}:{int(s.clean_xray_port_shadowsocks)}#SafeFlow-Shadowsocks"
    return "xray_shadowsocks.txt", url