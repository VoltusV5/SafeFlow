...
def _revoke_wg_peer(
    ctr: str, iface: str, conf: str, pubkey: str
) -> None:  # noqa: E302, E501
    # Remove from runtime
    subprocess.run(
        [
            ...
            "sh",
            "-c",
            f"wg set {iface} peer {pubkey} remove || true",
        ],
        check=False,
        timeout=30,
    )  # noqa: E501
    ...
    try:
        raw = subprocess.check_output(
            ["docker", "exec", ctr, "cat", conf], timeout=15
        )  # noqa: E501
        ...
        # Write back
        subprocess.run(
            ["docker", "exec", "-i", ctr, "sh", "-c", f"cat > {conf}"],
        ...
        logger.warning(
            f"Failed to remove peer {pubkey} from {conf} in {ctr}: {e}"
        )  # noqa: E501
...
"""
    subprocess.run(
        ["docker", "exec", ctr, "sh", "-c", cmd], check=False, timeout=30
    )  # noqa: E501
...
"""
    subprocess.run(
        ["docker", "exec", ctr, "sh", "-c", cmd], check=False, timeout=60
    )  # noqa: E501
        ...
        if key.protocol == VpnProtocol.AMNEZIA_WG.value:
            ...
            if ctr:
                ...
                await asyncio.to_thread(
                    _revoke_wg_peer, ctr, iface, conf, pubkey
                )  # noqa: E501
            ...
            if ctr2:
                ...
                await asyncio.to_thread(
                    _revoke_wg_peer, ctr2, iface2, conf2, pubkey
                )  # noqa: E501
            ...
            if ctr:
                iface = "wg0"
                conf = (
                    s.wireguard_wg_conf or "/opt/amnezia/wireguard/wg0.conf"
                ).strip()  # noqa: E501
                await asyncio.to_thread(
                    _revoke_wg_peer, ctr, iface, conf, pubkey
                )  # noqa: E501
            ...
            if not doc:
                return  # noqa: E701
            cfg_str = (
                doc.get("containers", [{}])[0].get("xray", {}).get("last_config", "")
            )  # noqa: E501
            if not cfg_str:
                return  # noqa: E701
            cfg = json.loads(cfg_str)
            uuid = cfg["outbounds"][0]["settings"]["vnext"][0]["users"][0][
                "id"
            ]  # noqa: E501
            if uuid:
                await asyncio.to_thread(_revoke_xray, ctr, uuid)
        elif key.protocol in (
            ...
            if key.protocol == VpnProtocol.OPENVPN.value:
                ...
                ctr = (
                    s.docker_openvpn_cloak_container or "amnezia-openvpn-cloak"
                ).strip()  # noqa: E501
            else:
                ctr = (
                    s.docker_shadowsocks_container or "amnezia-shadowsocks"
                ).strip()  # noqa: E501
            if not ctr:
                return  # noqa: E701
            url = key.key_value.strip()
            ...
            if not doc:
                return  # noqa: E701
            client_id = (
                doc.get("containers", [{}])[0].get("openvpn", {}).get("clientId", "")
            )  # noqa: E501
            if client_id:
            ...
            if not doc:
                return  # noqa: E701
            client_id = (
                doc.get("containers", [{}])[0].get("ikev2", {}).get("userName", "")
            )  # noqa: E501
            if client_id:
