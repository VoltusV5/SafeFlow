...
CONTACT_INTRO = (
    ...
    "Сообщения пока только у вас. Когда всё готово, нажмите «Подтвердить отправку» "
...
@router.message(_auth, StateFilter(ContactStates.composing), Command("cancel_contact"))
...
@router.message(_auth, StateFilter(ContactStates.composing), F.text == "Сообщить о проблеме")
async def contact_switch_to_report(message: Message, state: FSMContext) -> None:
    await state.clear()
    await message.answer(
...
async def contact_accumulate(message: Message, state: FSMContext) -> None:
    if message.text and message.text.startswith("/"):
        await message.answer(
            "Это похоже на команду. Добавьте обычные сообщения для администратора "
...
@router.callback_query(_auth, StateFilter(ContactStates.composing), F.data == "ct:abort")
...
@router.callback_query(_auth, StateFilter(ContactStates.composing), F.data == "ct:commit")
async def contact_cb_commit(cb: CallbackQuery, state: FSMContext) -> None:
    data = await state.get_data()
    ids: list[int] = list(data.get(CONTACT_IDS_KEY, []))
    if not ids:
        await cb.answer(
            "Пока нет сообщений. Напишите что-нибудь в чат, затем снова нажмите кнопку.",
            show_alert=True,
        )
        return
    ...
    uid = cb.from_user.id
    chat_user = cb.message.chat.id if isinstance(cb.message, Message) else cb.from_user.id
    ...
    if isinstance(cb.message, Message):
        if payload_ok:
            await cb.message.edit_text(
                f"Готово. В очереди было {len(ids)} ваших сообщений — они отправлены "
                f"администраторам."
            )
        elif header_ok:
            await cb.message.edit_text(
                "Администраторам ушли только ваш Telegram ID и username, "
                "но сами сообщения переслать не удалось (ограничение Telegram по типу контента). "
                "Попробуйте отправить текстом или напишите в поддержку из меню ещё раз."
            )
        else:
            await cb.message.edit_text(
                "Не удалось связаться с администраторами. "
                "Проверьте ADMIN_IDS в настройках бота и что админы не блокировали бота."
            )
    await cb.answer()
    if payload_ok:
        tail = "Готово. Если нужно, администратор ответит вам здесь в чате."
    elif header_ok:
        tail = "Проверьте сообщение выше — возможно, нужно повторить отправку текстом."
    else:
