...
def _script_env() -> dict[str, str]:
    ...
    use_v2 = (s.awg2_docker_container or "").strip() and (s.wg2_endpoint or "").strip()
    if use_v2:
        env["AWG_DOCKER_CONTAINER"] = s.awg2_docker_container.strip()
        env["AWG_WG_IFACE"] = (s.awg2_wg_iface or "wg0").strip() or "wg0"
        env["AWG_WG_CONF"] = (s.awg2_wg_conf or "/opt/amnezia/awg/wg0.conf").strip()
        env["WG_ENDPOINT"] = s.wg2_endpoint.strip()
        env["WG_CLIENT_IP_PREFIX"] = (s.wg2_client_ip_prefix or "10.8.2").strip() or "10.8.2"
        ...
        if ctr:
            ...
            env["AWG_WG_CONF"] = (s.awg_wg_conf or "/opt/amnezia/awg/wg0.conf").strip()
...
async def generate_amnezia_wg_peer(user_hint: str) -> tuple[str, str, str]:
    s = get_settings()
    ...
    if not use_v2:
        if not s.wg_endpoint.strip():
            raise PeerGenerationError("Задайте WG_ENDPOINT (публичный IP:порт UDP)")
        if not (s.awg_docker_container or "").strip() and (
            not s.wg_server_public_key.strip()
        ):
            raise PeerGenerationError(
                "Задайте WG_SERVER_PUBLIC_KEY или AWG_DOCKER_CONTAINER для авто-чтения ключа из контейнера"
            )
    ...
    try:
        ...
        raise PeerGenerationError("/bin/bash недоступен (нужен Linux для скрипта)") from None

    out_b, err_b = await proc.communicate()
    out = (out_b or b"").decode("utf-8", errors="replace").strip()
    err = (err_b or b"").decode("utf-8", errors="replace").strip()
    if proc.returncode != 0:
        logger.warning("generate_peer.sh rc=%s stderr=%s", proc.returncode, err)
        raise PeerGenerationError(err or f"скрипт завершился с кодом {proc.returncode}")

    data: dict[str, object] | None = None
    for raw_line in reversed([x.strip() for x in out.splitlines() if x.strip()]):
