...
class User(Base):
    ...
    # Колонка в SQLite исторически называется trial_start_date; на ключи VPN не влияет.
    ...
    da_donation_sub: Mapped["DonationAlertsSubscription | None"] = relationship(
        back_populates="user",
        uselist=False,
    )
    star_payments: Mapped[list[StarPayment]] = relationship(back_populates="user")
...
class VpnKey(Base):
    ...
    config_filename: Mapped[str] = mapped_column(String(255), default="config.txt")
...
class UserLimit(Base):
    ...
    limit_mbps: Mapped[int] = mapped_column(Integer(), default=500, server_default="500")
...
class DonationAlertsSubscription(Base):
    """Ежемесячное напоминание открыть Donation Alerts (оплата на стороне сайта)."""
