"""Таблицы только в analytics DB (см. session_analytics). user_id — ссылка на users.id логически, без FK."""

from __future__ import annotations

from datetime import date, datetime

from sqlalchemy import BigInteger, Date, DateTime, Float, Index, Integer, Text, func
...
class TrafficLog(BaseAnalytics):
    ...
    # Без него при 6M+ строках каждый SELECT ... WHERE user_id=X ORDER BY logged_at DESC LIMIT 1
    # делал temp B-tree sort по ~1.3M строкам на пользователя => 60% CPU в traffic_logger.
    __table_args__ = (
...
class WhitelistBypassFeedback(BaseAnalytics):
    """Нажатия «работает / не работает» у инструкции обхода белых списков (analytics DB)."""

    __tablename__ = "whitelist_bypass_feedback"

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), index=True)
    works: Mapped[bool] = mapped_column()  # True — «работает при белых», False — «не работает»


class XrayTrafficLog(BaseAnalytics):
    __tablename__ = "xray_traffic_log"
    # Составной индекс для быстрого поиска последней записи пользователя по Xray.
    __table_args__ = (
    ...
    uplink_delta: Mapped[int | None] = mapped_column(BigInteger(), nullable=True)
    downlink_delta: Mapped[int | None] = mapped_column(BigInteger(), nullable=True)
