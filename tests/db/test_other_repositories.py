...
async def test_server_repository(async_session):
    ...
    await repo.create({"country": "NL", "domain": "nl.example.com", "api_secret": "sec", "is_active": True})
    await repo.create({"country": "US", "domain": "us.example.com", "api_secret": "sec", "is_active": False})

    active = await repo.get_active_servers()
    assert len(active) == 1
    assert active[0].country == "NL"
...
async def test_key_repository(async_session):
    ...
    server = await server_repo.create({"country": "NL", "domain": "d", "api_secret": "s"})
